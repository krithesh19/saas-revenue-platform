...
def load_snapshot():
    return run_query("""
        SELECT
            ...
            (SELECT MAX(TOTAL_ARR)
             FROM SAAS_REVENUE_DB.GOLD.arr_summary)                         AS latest_arr,
            (SELECT COUNT(*)
             FROM SAAS_REVENUE_DB.GOLD.churn_analysis
             WHERE YEAR(CHURN_MONTH) = YEAR(CURRENT_DATE))                  AS churned_ytd,
            (SELECT ROUND(AVG(PROJECTED_LTV), 0)
             FROM SAAS_REVENUE_DB.GOLD.customer_ltv)                        AS avg_ltv,
            (SELECT SUM(MRR_CHANGE)
             FROM SAAS_REVENUE_DB.GOLD.revenue_waterfall
             WHERE YEAR(INVOICE_MONTH) = YEAR(CURRENT_DATE))                AS net_new_ytd
    """)
...
if not snapshot.empty:
    ...
    c5.metric("Net New MRR YTD", f"${net:,.0f}",
              delta=f"{'▲' if net > 0 else '▼'}")
else:
    st.warning("Could not load snapshot KPIs.")
...
|---|---|
| 💬 AI Revenue Analyst | Natural-language Q&A powered by LangChain + Groq LLaMA |
...
| 🌊 Revenue Waterfall | MRR movement analysis |
| 📈 Forecasting | 6-month MRR projection with 3 scenarios |
| 📊 Budget vs Actuals | Variance analysis with RAG status |
| 💰 Unit Economics | CAC, LTV:CAC ratio, payback period |
| 🔄 Cohort Analysis | LTV by acquisition cohort and segment |
| ⚡ Quick Ratio | Quick Ratio and Rule of 40 |
""")