"""
streamlit_app/pages/8_budget_vs_actual.py
...
budget_method = st.sidebar.radio(
    "Budget method",
    ["Auto (5% growth target)", "Manual (set per month)"]
)
...
if budget_method == "Auto (5% growth target)":
    ...
    budgets = [first_budget * ((1 + growth_target/100) ** i) for i in range(len(df))]
    df["budget_mrr"] = budgets
else:
    flat_budget = st.sidebar.number_input(
        "Flat monthly budget ($)", min_value=1000, value=int(df["actual_mrr"].mean()), step=1000)
...
fig2.update_layout(
    title="Monthly Variance (Actual - Budget)",
...
st.download_button(
    ...
    file_name="budget_vs_actuals.xlsx",
